...
| T2 | Flag smuggling past blocklist | Agent spells a blocked flag in a form a whole-token match misses: `--force=true`, a bundled short flag (`-an` = `-a -n`, `-uf`), or an abbreviated long flag (git accepts unambiguous prefixes: `--no-verif`, `--forc`) | 1 | 3 | 3 | ASVS V2 Validation | **Mitigated.** `BlockFlagsRule` matches the `=value`-stripped, case-folded name (`Flag.Base`), expands all-letter single-dash bundles into their short flags, and treats a long-flag token that is a prefix of a blocked long flag as that flag. All three widen what blocks, never what passes (over-matching such as `-mnote` fails safe). `--force-with-lease` is intentionally allowed on push (it is not a prefix of `--force`). Bundling and abbreviation were found open during the v12 review (see T3). |
| T3 | Git hook bypass | Agent passes `--no-verify` or `-n` to skip pre-commit hooks | 1 | 3 | 3 | ASVS V2 Validation | **Fully mitigated (since v12).** `git commit` carries a declared `BlockFlags(["--no-verify", "-n"])` policy evaluated at the `CommandDispatcher` seam; `--no-verify` is also in `PushBlockedFlags` for push. Before v12 this was bypassable — `safe git commit -an -m x` (bundled) and `--no-verif` (abbreviated) both skipped hooks, verified against real git — closed by the T2 hardening. |
...
| T7 | Work discarded via forced or resetting checkout | Agent runs `safe git checkout -B <existing>` (create-or-reset drops that branch's commits), `-f`/`--force`/`--discard-changes` (throws away uncommitted changes), or `-f -b <new>` (the `-b` dirty-tree exemption let the discard through) | 1 | 3 | 3 | ASVS V2 Validation / business-logic integrity | **Fully mitigated.** `checkout` blocks `-f`/`--force`/`--discard-changes` (case-folded) and `-B` (case-sensitive — `-b` is its legitimate twin), including bundled and abbreviated spellings (T2). Found while scoping `branch-delete`; both data-loss paths reproduced against real git before the fix. |
...
- I8: Mitigated by flag omission; the omissions rely on case-sensitive allowlist matching (E6, resolved)
...
| E6 | Allowlist bypass via case-folded short flag ([#27](https://github.com/hansen-consultancy/SafeCommands/issues/27)) | `Flag.Base` lowercases before matching, so a short flag in a subcommand allowlist also admits its uppercase twin. Where a tool distinguishes case (gh: `-r` reviewer vs `-R` repo, `-t` title vs `-T` template, `-f` field vs `-F` body-file), allowlisting the lowercase form silently grants the uppercase one | 1 | 3 | 3 | ASVS V2 Validation / V8 Authorization | **Fully mitigated.** Rules now normalize by failure direction: allowlists (`AllowSubcommandsRule`) and exemptions (`RequireCleanTreeRule`) match case-sensitively on `Flag.Name`, so a declared `-r` admits only `-r`; blocklists (`BlockFlags`) keep case-folded `Flag.Base` matching, which fails safe. A structural test walks every registered allowlist and asserts no flag's case twin is admitted unless listed explicitly, so the class cannot regress silently. The long-form-only workaround in the gh create allowlists was reverted (`-t`/`-r`/`-f`/`-B` restored). Side effect: `git checkout -B` no longer inherits the dirty-tree exemption of `-b`. Closed by [#27](https://github.com/hansen-consultancy/SafeCommands/issues/27). |
...
- E6: Resolved — allowlist and exemption flags match case-sensitively (`Flag.Name`), blocklists case-folded (`Flag.Base`); pinned by a registry-wide test
...
| **Git Safety** | Clean-tree checks, force-push blocked, hook bypass blocked, amend-after-push blocked, branch force-delete only after a content-already-merged proof, forced/resetting checkout (`-f`, `-B`) blocked; blocklists catch bundled (`-an`) and abbreviated (`--no-verif`) flags |
...
| v11 | 2026-09-23 | STRIDE update (case-sensitive allowlists) | E6 fully mitigated ([#27](https://github.com/hansen-consultancy/SafeCommands/issues/27)): allowlist and exemption matching is now case-sensitive (`Flag.Name`), blocklists stay case-folded (`Flag.Base`); a registry-wide test pins it. Likelihood 2 → 1, score 6 → 3. I8 text updated — its long-form-only workaround is gone. |
| v12 | 2026-09-23 | STRIDE update (blocklist smuggling + checkout) | Found that `BlockFlags` matched whole tokens only, so bundled short flags (`-an`) and git's abbreviated long options (`--no-verif`) bypassed every git blocklist — T3's hook-bypass block included, verified against real git. T2 hardened (bundle expansion, long-prefix match), score 6 → 3; T3 annotated. Added T7 (`checkout -B` reset, `-f`/`--force`/`--discard-changes` and `-f -b` discard), fully mitigated. |
