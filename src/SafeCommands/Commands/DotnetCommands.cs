...
static class DotnetCommands
{
    ...
    internal static int RunToolInstall(Ports p, string[] args) => Run.Tool(p, "dotnet", ["tool", "install", "-g", ..args]);

    internal static int RunAddPackage(Ports p, string[] args) => Run.Tool(p, "dotnet", ["add", "package", ..args]);

    internal static int RunAddReference(Ports p, string[] args) => Run.Tool(p, "dotnet", ["add", "reference", ..args]);

    internal static int RunNew(Ports p, string[] args) => Run.Tool(p, "dotnet", ["new", ..args]);
